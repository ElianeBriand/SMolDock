...
//    succeeded = mol.populateFromMol2File("../DockingTests/COX2_Ibuprofen/VINA_Cox2_BestRes_Charged.mol2", 120, modifiers); // Ibuprofen
    // succeeded = mol.populateFromMol2File("../DockingTests/hCES1/BENZIL_VinaBest_2.mol2", 120, postProcessors); // benzil
     succeeded = mol.populateFromSMILES("O=C(C(=O)c1ccccc1)c2ccc(CC3CCCCC3)cc2"); // Benzil
    //succeeded = mol.populateFromSMILES("O=C(C(=O)c1ccccc1)c2ccc(CCCC)cc2"); // Benzil


    mol.applyAtomVariant("[c,C][C:1](=O)[c,C]", sd::Atom::AtomVariant::covalentReversibleAcceptor);
